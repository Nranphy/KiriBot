...
class InstanceService:
    ...
    async def close(self) -> None:
        ...
        await asyncio.gather(*(self.process_client.stop(process) for process in processes))
    ...
    def _validate_config(self) -> None:
        for instance_id, instance in self.config.instances.items():
            ...
            if connection.protocol != 'onebot_v11':
                raise InvalidInstanceConfigError(f'实例 {instance_id} 当前只支持 OneBot v11 连接')
            if not instance.working_directory.is_dir():
                raise InvalidInstanceConfigError(f'实例 {instance_id} 的工作目录不存在: {instance.working_directory}')
